...
function getphotogallery_func( $atts, $content = null ) {
	...
	if($debugphotogallery){
		...
		$taxonomy_objects = get_object_taxonomies( 'photogallery', 'objects' );
	...
	if($a['format']== 'masonry'){
		$output .='<div class="photogallery-masonry row">';
		$output .= '<div class="grid-sizer"></div>';
	}elseif($a['format']== 'grid'){
		$output .='<div class="photogallery-grid row">';
	}elseif($a['format']== 'list'){
		$output .='<div class="photogallery-list">';
	}
	...
	foreach($photogallery as $photo){
		...
		$category = wp_get_post_terms( $photoid, 'photogallery_category');
		...
		$photogallery_large_url = $large[0];


		$output .= '<div class="photogallery-item '.$a['class'].' photo-'.$photoid.'">';
		if($photogallery_thumb_url){
			$output .= '<a href="'.$photogallery_large_url.'" class="thumbnail" title="'.$alt.'" data-lightbox="lightbox"><img src="'.$photogallery_thumb_url.'" alt="'.$alt.'" class="img-fluid" /></a>';
		}
		$output .= '</div>';
		$i++;
	}
